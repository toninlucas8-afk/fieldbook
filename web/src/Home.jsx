...
// l'elenco dei lavori in faccia appena si entra.
export default function Home ({ utente, segnale, apriLavoro, apriNuovo, apriNote, apriLavori, apriMenu, vaiA }) {
  ...
  const ricarica = useCallback(async () => {
    try {
      setR(await api.riassunto(giornoISO()))
      setErrore('')
    ...
    <>
      ...
      <div className="contenuto con-barra">
        ...
        {!r
          ...
          : (
            <>
              <OggiWidget lavori={r.oggi} apriLavoro={apriLavoro} apriNuovo={apriNuovo} />
              ...
              <NoteWidget note={r.note} apriNote={apriNote} />

              {r.ultime_foto.some((f) => !rotte.has(f.id)) && (
                <>
...
function OggiWidget ({ lavori, apriLavoro, apriNuovo }) {
  if (lavori.length === 0) {
    return (
...
function NoteWidget ({ note, apriNote }) {
  return (
    <>
      ...
      <button className="carta note-sintesi" onClick={apriNote}>
        {note.quante === 0
          ...
          : (
            <>
              <ul>
                {note.prime.map((n) => <li key={n.id}>{n.testo}</li>)}
              </ul>
              {note.quante > note.prime.length && (
                <small>e altre {note.quante - note.prime.length}</small>
              )}
            </>
            )}
      </button>
